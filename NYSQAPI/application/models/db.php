...
class dbModel{
    ...
    //['x','y']
    public function preprocessing($sql,$arr){
        $stmt= $this->yafpdo->prepare($sql);
    ...
    //多条记录查询预处理
    //参数一： SELECT * FROM USER WHERE user = ？ and pass = ?
    //参数二：['x','y']
    public function stmtFetchAll($sql,$arr){
        $stmt= $this->yafpdo->prepare($sql);
        for($i=0;$i<count($arr);$i++){
            $stmt->bindValue(($i+1),$arr[$i]);
        }
    ...
    //参数一： SELECT * FROM USER WHERE user = ？ and pass = ?
    //参数二：['x','y']
    public function stmtFetch($sql,$arr){
        $stmt= $this->yafpdo->prepare($sql);
        for($i=0;$i<count($arr);$i++){
            $stmt->bindValue(($i+1),$arr[$i]);
        }
    ...
    //修改，添加，删除预处理
    //参数一： INSERT INTO USER VALUES(NULL,?,?)
    //参数二：['x','y']
    public function stmtExecute($sql,$arr){
        $stmt= $this->yafpdo->prepare($sql);
        for($i=0;$i<count($arr);$i++){
            $stmt->bindValue(($i+1),$arr[$i]);
        }
