      ...
      <el-table :data="list" stripe style="width: 100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="danger" @click="handleRemove(scope.row.m_id)">清除</el-button>
...
<script>
import { statusFilter, removeMerc, status } from '@/api/merc'
...
export default {
  ...
  methods: {
    ...
    handleRemove(row) {
      this.jsonData.id = row
      removeMerc(this.jsonData).then(response => {
        if (response.data === 'success') {
          this.$message.success('操作成功')
          setTimeout(() => {
            this.reload()
